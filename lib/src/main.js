...
const init = () => {
    ...
    const actions = document.querySelectorAll(`[${prefix}-action]`);
    for (const action of actions) {
        if (!action.hasAttribute(`${prefix}-data`) && !action.closest(`[${prefix}-collection]`)) {
            action.setAttribute(`${prefix}-data`, '');
        }
    }
            ...
            switch (action) {
                case 'create': {
                    try {
                        ...
                        await db.collection(collection).create(formData);
                    } catch (error) {
                        console.error(error);
                    }
                    break;
    ...
    register(CommradInput, `${prefix}-input`, ['type'], { shadow: true, mode: 'closed' });
